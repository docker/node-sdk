...
test(
    'should receive container stdout on attach',
    { timeout: 30000 },
    async () => {
        ...
        try {
            ...
            console.log(
                `    Container created: ${containerId.substring(0, 12)}`,
            );
            ...
            console.log(
                `    Captured stdout data: ${JSON.stringify(stdoutData)}`,
            );
            console.log(
                `    Captured stderr data: ${JSON.stringify(stderrData)}`,
            );

            // Check that we received "hello" in stdout
            const allStdout = stdoutData.join('');
            assert.ok(
                allStdout.includes('hello'),
                'Should receive "hello" in stdout',
            );
...
test(
    'should collect container output using containerLogs',
    { timeout: 30000 },
    async () => {
        const client = await DockerClient.fromDockerConfig();
        let containerId: string | undefined;
        ...
        try {
            ...
            console.log(
                '  Creating Alpine container with multi-line output...',
            );
            const createResponse = await client.containerCreate({
                Image: 'alpine',
            ...
            assert.ok(
                allStdoutLogs.includes('line1'),
                'Should receive "line1" in stdout logs',
            );
            assert.ok(
                allStdoutLogs.includes('line2'),
                'Should receive "line2" in stdout logs',
            );
            assert.ok(
                allStdoutLogs.includes('line3'),
...
test(
    'container lifecycle should work end-to-end',
    { timeout: 30000 },
    async () => {
        const client = await DockerClient.fromDockerConfig();
        let containerId: string | undefined;
        ...
        try {
            ...
            assert.notStrictEqual(filteredContainers, null);
            const foundContainer = filteredContainers.find(
                (c) => c.Id === containerId,
            );
            assert.notStrictEqual(foundContainer, null);
            console.log(
            ...
            assert.notStrictEqual(inspectResponse, null);
            assert.strictEqual(inspectResponse.State?.Running, true);
            console.log(
                `    Container state: ${inspectResponse.State?.Status}`,
            );
            ...
            assert.strictEqual(pausedInspect.State?.Paused, true);
            console.log(
            ...
            assert.strictEqual(unpausedInspect.State?.Running, true);
            assert.strictEqual(unpausedInspect.State?.Paused, false);
            console.log(
            ...
            assert.strictEqual(restartedInspect.State?.Running, true);
            console.log(
            ...
            assert.strictEqual(finalInspect.State?.Running, false);
            ...
            assert.notStrictEqual(stoppedContainer, null);
            console.log(
